...
@Composable
fun IncomingDeliveriesListScreen(
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            ...
            when (val s = state) {
                ...
                is UiState.Success -> {
                    if (s.data.isEmpty()) {
                        EmptyState(
                            message = if (showCompleted) "ВХП нет" else "Нет ВХП к выполнению",
                            icon = Icons.Filled.Inbox,
                        )
                    } else {
                        LazyColumn(contentPadding = PaddingValues(Spacing.md)) {
                            items(s.data, key = { it.id }) { delivery ->
                                IncomingDeliveryRow(delivery, onClick = { onOpenDelivery(delivery.id, delivery.number) })
                            }
                        }
                    }
                }
...
private fun IncomingDeliveryRow(delivery: IncomingDelivery, onClick: () -> Unit) {
    AppCard(
        ...
        Row(
            ...
            delivery.plannedDate?.let { Text(it.take(10)) }
        }
        if (delivery.isCrossDock) {
            Text("Кросс-докинг", style = androidx.compose.material3.MaterialTheme.typography.labelSmall)
        }
    }
}
