...
export interface OutgoingDeliveryParseResult {
...
/**
 * Парсит файл ИСП: шапка (склад отгрузки, КД, дата отгрузки) и таблицу
 * позиций «#ИСП | Артикул | Наименование товара | Кол-во | Вес | Объем».
 */
export function parseOutgoingDeliveryExcel(buffer: ArrayBuffer): OutgoingDeliveryParseResult {
  const wb = XLSX.read(buffer, { type: 'array', cellDates: true });
  const sheetName = wb.SheetNames.find((n) => norm(n).includes('исп')) ?? wb.SheetNames[0];
  const sheet = wb.Sheets[sheetName];
  if (!sheet) throw new Error('Не найден лист с данными ИСП');
  ...
  for (const row of grid.slice(0, 8)) {
    for (let c = 0; c < row.length - 1; c++) {
      ...
      if (key.includes('складотгрузк')) warehouseCode = toStr(row[c + 1]) ?? warehouseCode;
      if (key === 'кд') isCrossDock = toBool(row[c + 1]);
      if (key.includes('датаотгрузк')) shipDate = toIsoDate(row[c + 1]) ?? shipDate;
    }
  }
  ...
  const items: OutgoingDeliveryItemInput[] = [];
  ...
  return { warehouseCode, isCrossDock, shipDate, items, warnings };
}
